...
// Spotify Otomatik Token Alma ve Yenileme
async function spotifyTokenAl() {
    try {
        const data = await spotifyApi.clientCredentialsGrant();
        spotifyApi.setAccessToken(data.body['access_token']);
        console.log('🎵 Spotify Token başarıyla alındı ve aktifleşti!');
        // Token süresi dolmadan (1 saatte bir) otomatik tazeler
        setTimeout(spotifyTokenAl, (data.body['expires_in'] - 60) * 1000);
    } catch (err) {
        console.error('❌ Spotify token alınırken hata oluştu:', err.message);
    }
}

// Render Uptime & Port Dinleyici (Render çökmesini engellemek için)
...
client.once('ready', () => {
  ...
  spotifyTokenAl(); // Bot açıldığında Spotify bağlantısını başlatır
});

// Anlık hatalarda botun çökmesini engelleme
...
client.on('messageCreate', async (message) => {
    ...
    if (command === '!yardım' || command === '!help') {
        const helpEmbed = new EmbedBuilder()
            ...
            .addFields(
                { 
                    name: '🎵 Spotify & Müzik Komutları', 
                    value: '`!şarkı-ara <isim>` • Spotify\'da şarkı arar.\n`!çal <şarkı adı>` • Spotify şarkısını getirir ve detaylarını gösterir.\n`!liste-ara <isim>` • Çalma listesi arar.' 
                },
                { 
    ...
    // 🎵 SPOTIFY KOMUTLARI
    if (command === '!şarkı-ara' || command === '!sarki-ara' || command === '!ara') {
        const query = args.join(' ');
        if (!query) return message.reply('❌ Aratmak istediğin şarkının adını yazmalısın kanka!');

        try {
            const res = await spotifyApi.searchTracks(query, { limit: 5 });
            const tracks = res.body.tracks.items;

            if (!tracks || tracks.length === 0) return message.reply('❌ Spotify\'da böyle bir şarkı bulunamadı!');

            const embed = new EmbedBuilder()
                .setTitle(`🔍 Spotify Arama Sonuçları: "${query}"`)
                .setColor('#1DB954')
                .setThumbnail(tracks[0].album.images[0]?.url || null);

            let desc = '';
            tracks.forEach((t, i) => {
                desc += `**${i + 1}.** [${t.name}](${t.external_urls.spotify}) - ${t.artists.map(a => a.name).join(', ')}\n`;
            });
            embed.setDescription(desc);

            return message.reply({ embeds: [embed] });
        } catch (e) {
            console.error('Spotify arama hatası:', e);
            return message.reply('❌ Spotify\'da arama yapılırken bir sorun oluştu!');
        }
    }

    if (command === '!çal' || command === '!cal') {
        const query = args.join(' ');
        if (!query) return message.reply('❌ Çalmak istediğin şarkının adını yaz kanka!');

        try {
            const res = await spotifyApi.searchTracks(query, { limit: 1 });
            const track = res.body.tracks.items[0];

            if (!track) return message.reply('❌ Şarkı bulunamadı!');
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor('#1DB954')
                .addFields(
                    { name: '👤 Sanatçı', value: track.artists.map(a => a.name).join(', '), inline: true },
                    { name: '💽 Albüm', value: track.album.name, inline: true },
                    { name: '⏱️ Süre', value: `${Math.floor(track.duration_ms / 60000)}:${Math.floor((track.duration_ms % 60000) / 1000).toString().padStart(2, '0')}`, inline: true }
                )
                .setThumbnail(track.album.images[0]?.url || null)
                .setFooter({ text: 'Spotify Entegrasyonu' });

            return message.reply({ content: `🎵 **İşte Spotify Linki:** ${track.external_urls.spotify}`, embeds: [embed] });
        } catch (e) {
            console.error('Spotify çal hatası:', e);
            return message.reply('❌ Şarkı getirilirken hata oluştu!');
        }
    }

    if (command === '!liste-ara' || command === '!playlist-ara') {
        const query = args.join(' ');
        if (!query) return message.reply('❌ Aratmak istediğin playlist adını yaz kanka!');

        try {
            const res = await spotifyApi.searchPlaylists(query, { limit: 3 });
            const playlists = res.body.playlists.items;

            if (!playlists || playlists.length === 0) return message.reply('❌ Spotify\'da böyle bir liste bulunamadı!');

            const embed = new EmbedBuilder()
                .setTitle(`🎧 Spotify Çalma Listeleri: "${query}"`)
                .setColor('#1DB954');

            let desc = '';
            playlists.forEach((p, i) => {
                if (p) desc += `**${i + 1}.** [${p.name}](${p.external_urls.spotify}) - *Sahibi: ${p.owner.display_name}*\n`;
            });
            embed.setDescription(desc);

            return message.reply({ embeds: [embed] });
        } catch (e) {
            console.error('Playlist arama hatası:', e);
            return message.reply('❌ Çalma listesi aranırken hata oluştu!');
        }
    }
...
client.login(process.env.TOKEN).catch(err => {
    console.error("❌ TOKEN HATASI! Render Environment alanındaki TOKEN değişkenini kontrol et:", err);
});

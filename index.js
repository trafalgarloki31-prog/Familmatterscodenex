...
function getUser(userId) {
    if (!db.users[userId]) {
        db.users[userId] = {
            ...
            marriedSince: null,
            ring: null,
            inventory: { altin: 0, gumus: 0, elmas: 0, uranyum: 0, dol: 0, netherit: 0, su: 0, ametisturanyum: 0 },
            lastWork: 0,
            house: null,
            vehicle: null,
            children: [],
    ...
    if (!db.users[userId].inventory.ametisturanyum) db.users[userId].inventory.ametisturanyum = 0;
    ...
    if (db.users[userId].protection === undefined) db.users[userId].protection = false;
    if (db.users[userId].haracVeren === undefined) db.users[userId].haracVeren = null;
    if (!db.users[userId].haracAlanlar) db.users[userId].haracAlanlar = [];
    if (db.users[userId].guaranteedWin === undefined) db.users[userId].guaranteedWin = false;
    return db.users[userId];
}

// İş listesi (sıralama = maaş sıralaması)
const JOBS = {
    'dinlenme': { name: 'Dinlenme', minLevel: 1, basePay: 100, flavor: 'biraz dinlenip kendine geldin' },
    'sanayi': { name: 'Sanayi İşçisi', minLevel: 8, basePay: 200, flavor: 'ustana yardım ettin' },
    'sahte ayakkabıcı': { name: 'Sahte Ayakkabıcı', minLevel: 15, basePay: 450, flavor: 'sahte ayakkabıları müşterilere kaydırdın' },
    'eskort': { name: 'Eskort', minLevel: 22, basePay: 800, flavor: 'müşterine eşlik ettin' },
    'spotify sanatçısı': { name: 'Spotify Sanatçısı', minLevel: 30, basePay: 1500, flavor: 'yeni bir single yayınladın' },
    'esnaf': { name: 'Esnaf', minLevel: 38, basePay: 2200, flavor: 'dükkanında müşterilerle pazarlık ettin' },
    'şirket çalışanı': { name: 'Şirket Çalışanı', minLevel: 48, basePay: 3500, flavor: 'toplantıda sunum yaptın' },
    'patron': { name: 'Patron', minLevel: 58, basePay: 5500, flavor: 'çalışanlarına talimat verdin' },
    'yatırımcı': { name: 'Yatırımcı', minLevel: 70, basePay: 9000, flavor: 'borsada büyük bir hamle yaptın' },
    'büyük patron': { name: 'Büyük Patron', minLevel: 85, basePay: 15000, flavor: 'şirket imparatorluğunu genişlettin' },
    'ak parti milletvekili': { name: 'AK Parti Milletvekili', minLevel: 110, basePay: 25000, flavor: 'mecliste konuşma yaptın' },
    'cumhurbaşkanı': { name: 'Cumhurbaşkanı', minLevel: 150, basePay: 500000000, flavor: 'ülkeyi yönettin', fameBonus: 200 }
};
...
const CHILD_MAX = 50; // en fazla 50 çocuk sahibi olunabilir
...
const MARKET = {
    ...
    ametisturanyum: { name: 'Ametistlenmiş Döllenmiş Uranyum', emoji: '🟣', price: 300000000, xpBonus: 1.0 }
};

function updateMarket() {
    for (const key in MARKET) {
        // Çok uçuk dalgalanma: -%25 ile +%25 arası
        const changePercent = (Math.random() * 50 - 25) / 100;
        MARKET[key].price = Math.max(10, Math.round(MARKET[key].price * (1 + changePercent)));
    }
}
setInterval(updateMarket, 10 * 1000); // Fiyatlar her 10 saniyede bir güncellenir
...
// ⛓️ KÖLE SİSTEMİ
// -------------------------------------------------------------
// Her köle, sahibine dakikada bir %50 ihtimalle 1 uranyum kazandırır.
const SLAVE_MARKET_PRICE = 25000; // pazardan doğrudan köle satın alma fiyatı (adet başı)
setInterval(() => {
    for (const userId in db.users) {
        const u = db.users[userId];
...
// 💰 Haraç sistemi: şöhreti yüksek olanlar düşük şöhretlileri haraca bağlayabilir
const HARAC_ORAN = 0.20;

function haracKesintisiUygula(u, kazanc) {
    if (!u.haracVeren) return { netKazanc: kazanc, haracMiktari: 0, haracAlanId: null };
    const haracAlan = getUser(u.haracVeren);
    const haracMiktari = Math.round(kazanc * HARAC_ORAN);
    haracAlan.balance += haracMiktari;
    return { netKazanc: kazanc - haracMiktari, haracMiktari, haracAlanId: u.haracVeren };
}
...
const mafias = {}; // mafyaId -> { name, bossId, members: [], balance, lastGelir, gangMembers: [], channelId }

// 🛡️ Tetikçiye karşı koruma
const PROTECTION_PRICE = 75000;
const PROTECTION_REDUCTION = 0.5; // korumalıyken suikast başarı şansı yarıya iner

// 👥 Çete üyeleri (NPC)
const GANG_MEMBER_PRICE = 2000000;
const GANG_MEMBER_MAX = 50;
const GANG_INCOME_MIN = 10000000;
const GANG_INCOME_MAX = 20000000;
const GANG_ARREST_CHANCE = 0.40;
const GANG_FIRST_NAMES = ['Kemal', 'Hasan', 'Recep', 'İbrahim', 'Mehmet', 'Ali', 'Veli', 'Mustafa', 'Cengiz', 'Tarık', 'Selim', 'Orhan', 'Burak', 'Kadir', 'Yusuf', 'Ekrem', 'Nihat', 'Ferhat', 'Aziz', 'Cemal'];

function raslantiCeteUyesiIsmi(mafyaAdi) {
    const isim = GANG_FIRST_NAMES[Math.floor(Math.random() * GANG_FIRST_NAMES.length)];
    return `${isim} ${mafyaAdi}`;
}

// Çete üyeleri her 5 dakikada bir ya tutuklanır/öldürülür ya da hazineye gelir getirir
setInterval(() => {
    for (const mafyaId in mafias) {
        const m = mafias[mafyaId];
        if (!m.gangMembers || m.gangMembers.length === 0) continue;

        const kalanlar = [];
        const tutuklananlar = [];
        for (const uye of m.gangMembers) {
            if (Math.random() < GANG_ARREST_CHANCE) {
                const kader = Math.random() < 0.5 ? 'tutuklandı' : 'öldürüldü';
                tutuklananlar.push(`${uye.name} (${kader})`);
            } else {
                const gelir = Math.random() < 0.5 ? GANG_INCOME_MIN : GANG_INCOME_MAX;
                m.balance += gelir;
                kalanlar.push(uye);
            }
        }
        m.gangMembers = kalanlar;

        if (tutuklananlar.length > 0 && m.channelId) {
            const channel = client.channels.cache.get(m.channelId);
            if (channel) {
                channel.send(`🚨 **${m.name}** mafyasından haber var: ${tutuklananlar.join(', ')}! Çete artık **${m.gangMembers.length}** üyeye düştü.`).catch(() => {});
            }
        }
    }
}, 5 * 60 * 1000);
...
client.on('messageCreate', async (message) => {
    ...
    if (command === '!yardım' || command === '!help') {
        const helpEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                {
                    name: '🕴️ Mafya',
                    value: '`!mafya kur <isim>` • Mafya kurar.\n`!mafya katıl @patron` • Bir mafyaya katılır.\n`!mafya ayrıl` • Mafyadan ayrılır.\n`!mafya bilgi` • Mafya bilgisi.\n`!mafya çete al <adet>` • Çete üyesi satın alır (patron).\n`!mafya topla` • Mafya hazinesini toplar (patron).\n`!mafya düello @patron` • İki patron kapışır, kaybeden mafyasını kaybeder.'
                },
                {
                    name: '💰 Haraç & 🛡️ Koruma',
                    value: '`!haraç bağla @kişi` • Şöhretin yüksekse kişiyi haraca bağlarsın (%20 vergi).\n`!haraç kaldır` • Verdiğin haracı kaldırır (şöhretin geçmeli).\n`!haraç bilgi` • Haraç durumunu gösterir.\n`!koruma al` • Tetikçiye karşı koruma satın alır.'
                }
            )
    ...
    if (command === '!çalış' || command === '!calis') {
        ...
        const { netKazanc, haracMiktari, haracAlanId } = haracKesintisiUygula(userData, kazanc);
        userData.balance += netKazanc;
        const haracNotu = haracMiktari > 0 ? `\n💰 <@${haracAlanId}> kişisine haraç olarak **${haracMiktari.toLocaleString()} FC** gitti.` : '';
        return message.reply(`💼 **${job.name}** olarak **${job.flavor}** ve **${netKazanc.toLocaleString()} FC** kazandın! (+${sohretKazanci} 🌟 şöhret)${haracNotu}`);
    }
    ...
    if (command === '!otoçalış' || command === '!otocalis') {
        ...
        const intervalId = setInterval(() => {
            ...
            if (u.job === 'eskort') {
                ...
                const { netKazanc } = haracKesintisiUygula(u, kazanc);
                u.balance += netKazanc;
            }
            u.lastWork = Date.now();
        }, 30 * 1000);
    ...
    // 🤖 GİZLİ HİLE KOMUTLARI (yardım listesinde görünmez)
    if (command === '!şöhret3x' || command === '!sohret3x') {
        userData.fame = userData.fame * 3;
        return message.reply(`🌟 Şöhretin 3 katına çıkarıldı! Yeni şöhret: **${userData.fame.toLocaleString()}**`);
    }

    if (command === '!levelup') {
        const adet = parseInt(args[0]) || 1;
        userData.level += adet;
        return message.reply(`⭐ **${adet}** level atladın! Yeni level: **${userData.level}**`);
    }

    if (command === '!fc5x') {
        userData.balance = userData.balance * 5;
        return message.reply(`💰 Bakiyen 5 katına çıkarıldı! Yeni bakiye: **${userData.balance.toLocaleString()} FC**`);
    }

    if (command === '!kumarzafer') {
        userData.guaranteedWin = true;
        return message.reply(`🎲 Bir sonraki \`!kumarhane\` maçında **%100** kazanacaksın!`);
    }
    ...
    if (command === '!çocuk' || command === '!cocuk') {
        ...
        if (userData.children.length >= CHILD_MAX) return message.reply(`❌ En fazla **${CHILD_MAX}** çocuğa sahip olabilirsin!`);
    ...
    if (command === '!köle' || command === '!kole') {
        ...
        if (sub === 'al') {
            const adet = args[1] ? parseInt(args[1]) : 1;
            if (!adet || adet <= 0) return message.reply('❌ Doğru kullanım: `!köle al <adet>` (adet belirtmezsen 1 alınır)');
            const toplamMaliyet = SLAVE_MARKET_PRICE * adet;
            if (userData.balance < toplamMaliyet) return message.reply(`❌ Yeterli bakiyen yok! Gereken: **${toplamMaliyet.toLocaleString()} FC**`);
            userData.balance -= toplamMaliyet;
            for (let i = 0; i < adet; i++) {
                userData.slaves.push({ source: 'pazar', acquiredAt: Date.now() });
            }
            return message.reply(`⛓️ Pazardan **${adet}** köle satın aldın! (**${toplamMaliyet.toLocaleString()} FC**) Her biri dakikada bir %50 ihtimalle **1 ☢️ Uranyum** kazandırıyor.`);
        }

        if (sub === 'sat') {
            const adet = args[1] ? parseInt(args[1]) : 1;
            if (!userData.slaves || userData.slaves.length === 0) return message.reply('❌ Hiç kölen yok!');
            if (!adet || adet <= 0) return message.reply('❌ Doğru kullanım: `!köle sat <adet>`');
            const satilacakAdet = Math.min(adet, userData.slaves.length);
            userData.slaves.splice(0, satilacakAdet);
            const satisFiyati = Math.round(SLAVE_MARKET_PRICE * 0.5) * satilacakAdet;
            userData.balance += satisFiyati;
            return message.reply(`✅ **${satilacakAdet}** köleyi sattın ve **${satisFiyati.toLocaleString()} FC** kazandın.`);
        }

        return message.reply(`❌ Doğru kullanım: \`!köle al <adet>\` (pazardan satın al, adet başı ${SLAVE_MARKET_PRICE.toLocaleString()} FC) veya \`!köle sat <adet>\``);
    }

    if (command === '!kölelerim' || command === '!kolelerim') {
        if (!userData.slaves || userData.slaves.length === 0) return message.reply('⛓️ Hiç kölen yok.');
        return message.reply(`⛓️ Toplam **${userData.slaves.length}** kölen var. Her biri dakikada bir %50 ihtimalle 1 ☢️ Uranyum kazandırıyor.`);
    }
    ...
    if (command === '!kumarhane') {
        ...
        if (sub === 'kabul') {
            ...
            // Kazananı belirle: %100 kazanma hilesi varsa öncelik ona, yoksa %50-%50
            let defenderKazandi;
            if (defenderData.guaranteedWin) {
                defenderKazandi = true;
                defenderData.guaranteedWin = false;
            } else if (challengerData.guaranteedWin) {
                defenderKazandi = false;
                challengerData.guaranteedWin = false;
            } else {
                defenderKazandi = Math.random() < 0.5;
            }
    ...
    if (command === '!mafya') {
        ...
        if (sub === 'kur') {
            ...
            mafias[mafyaId] = { name: isim, bossId: message.author.id, members: [message.author.id], balance: 0, lastGelir: 0, gangMembers: [], channelId: message.channel.id };
        ...
        if (sub === 'ayrıl' || sub === 'ayril') {
            if (!userData.mafia || !mafias[userData.mafia]) return message.reply('❌ Bir mafyaya üye değilsin!');
            const m = mafias[userData.mafia];

            if (m.bossId === message.author.id) {
                // Patron ayrılırsa mafya tamamen dağılır
                for (const memberId of m.members) {
                    const memberData = getUser(memberId);
                    memberData.mafia = null;
                }
                delete mafias[userData.mafia];
                return message.reply(`💥 Patron olarak ayrıldın, **${m.name}** mafyası tamamen dağıldı!`);
            }

            m.members = m.members.filter(id => id !== message.author.id);
            userData.mafia = null;
            return message.reply(`👋 **${m.name}** mafyasından ayrıldın.`);
        }
        ...
        if (sub === 'çete' || sub === 'cete') {
            if (!userData.mafia || !mafias[userData.mafia]) return message.reply('❌ Bir mafyaya üye değilsin!');
            const m = mafias[userData.mafia];
            if (m.bossId !== message.author.id) return message.reply('❌ Sadece patron çete üyesi satın alabilir!');

            const alSub = args[1]?.toLowerCase();
            if (alSub !== 'al') return message.reply(`❌ Doğru kullanım: \`!mafya çete al <adet>\` (üye başı ${GANG_MEMBER_PRICE.toLocaleString()} FC, maksimum ${GANG_MEMBER_MAX} üye)`);

            const adet = parseInt(args[2]) || 1;
            if (adet <= 0) return message.reply('❌ Geçerli bir adet gir!');
            if (m.gangMembers.length + adet > GANG_MEMBER_MAX) {
                return message.reply(`❌ Bir mafyanın en fazla **${GANG_MEMBER_MAX}** çete üyesi olabilir! (Şu an: ${m.gangMembers.length})`);
            }

            const toplamMaliyet = GANG_MEMBER_PRICE * adet;
            if (userData.balance < toplamMaliyet) return message.reply(`❌ Yeterli bakiyen yok! Gereken: **${toplamMaliyet.toLocaleString()} FC**`);

            userData.balance -= toplamMaliyet;
            const yeniUyeler = [];
            for (let i = 0; i < adet; i++) {
                const isim = raslantiCeteUyesiIsmi(m.name);
                m.gangMembers.push({ name: isim, joinedAt: Date.now() });
                yeniUyeler.push(isim);
            }

            return message.reply(`👥 **${adet}** çete üyesi satın aldın! (**${toplamMaliyet.toLocaleString()} FC**)\nYeni üyeler: ${yeniUyeler.join(', ')}\nHer üye 5 dakikada bir hazineye **10M-20M FC** getirir, ama %${GANG_ARREST_CHANCE * 100} ihtimalle tutuklanır ya da öldürülür.`);
        }

        if (sub === 'topla') {
            if (!userData.mafia || !mafias[userData.mafia]) return message.reply('❌ Bir mafyaya üye değilsin!');
            const m = mafias[userData.mafia];
            if (m.bossId !== message.author.id) return message.reply('❌ Sadece patron hazineyi toplayabilir!');
            if (m.balance <= 0) return message.reply('❌ Hazinede hiç para yok!');

            const toplananPara = m.balance;
            m.balance = 0;
            userData.balance += toplananPara;
            return message.reply(`💰 Mafya hazinesinden **${toplananPara.toLocaleString()} FC** topladın!`);
        }
        ...
        return message.reply('❌ Doğru kullanım: `!mafya kur <isim>` | `!mafya katıl @patron` | `!mafya ayrıl` | `!mafya bilgi` | `!mafya gelir` | `!mafya çete al <adet>` | `!mafya topla` | `!mafya düello @patron`');
    }

    // =========================================================
    // 💰 HARAÇ (şöhret üstünlüğüyle vergi bağlama)
    // =========================================================
    if (command === '!haraç' || command === '!harac') {
        const sub = args[0]?.toLowerCase();

        if (sub === 'bağla' || sub === 'bagla') {
            const target = message.mentions.users.first();
            if (!target) return message.reply('❌ Doğru kullanım: `!haraç bağla @kişi`');
            if (target.id === message.author.id) return message.reply('❌ Kendine haraç bağlayamazsın!');

            const targetData = getUser(target.id);
            if (userData.fame <= targetData.fame) return message.reply(`❌ Bu kişiyi haraca bağlayamazsın, şöhretin onunkinden yüksek olmalı! (Senin: ${userData.fame}, Onun: ${targetData.fame})`);
            if (targetData.haracVeren === message.author.id) return message.reply('❌ Bu kişi zaten sana haraç veriyor!');

            targetData.haracVeren = message.author.id;
            if (!userData.haracAlanlar.includes(target.id)) userData.haracAlanlar.push(target.id);

            return message.reply(`💰 <@${target.id}> kişisini haraca bağladın! Artık çalıştığında maaşının **%${HARAC_ORAN * 100}**'i sana gidecek.`);
        }

        if (sub === 'kaldır' || sub === 'kaldir') {
            if (!userData.haracVeren) return message.reply('❌ Kimseye haraç vermiyorsun!');
            const haracAlanData = getUser(userData.haracVeren);
            if (userData.fame <= haracAlanData.fame) {
                return message.reply('❌ Haracı kaldıramazsın, senin şöhretin onunkini geçmeli!');
            }
            const eskiHaracAlanId = userData.haracVeren;
            userData.haracVeren = null;
            const eskiHaracAlan = getUser(eskiHaracAlanId);
            eskiHaracAlan.haracAlanlar = eskiHaracAlan.haracAlanlar.filter(id => id !== message.author.id);
            return message.reply(`✅ Artık <@${eskiHaracAlanId}> kişisine haraç vermiyorsun!`);
        }

        if (sub === 'bilgi') {
            let metin = '';
            metin += userData.haracVeren ? `💸 <@${userData.haracVeren}> kişisine haraç veriyorsun.\n` : '💸 Kimseye haraç vermiyorsun.\n';
            metin += (userData.haracAlanlar && userData.haracAlanlar.length > 0)
                ? `💰 Haraç aldığın kişiler: ${userData.haracAlanlar.map(id => `<@${id}>`).join(', ')}`
                : '💰 Kimseden haraç almıyorsun.';
            return message.reply(metin);
        }

        return message.reply('❌ Doğru kullanım: `!haraç bağla @kişi` | `!haraç kaldır` | `!haraç bilgi`');
    }

    // =========================================================
    // 🛡️ KORUMA (tetikçiye karşı)
    // =========================================================
    if (command === '!koruma') {
        const sub = args[0]?.toLowerCase();
        if (sub === 'al') {
            if (userData.protection) return message.reply('❌ Zaten korumalısın!');
            if (userData.balance < PROTECTION_PRICE) return message.reply(`❌ Yeterli bakiyen yok! Gereken: **${PROTECTION_PRICE.toLocaleString()} FC**`);
            userData.balance -= PROTECTION_PRICE;
            userData.protection = true;
            return message.reply(`🛡️ Koruma satın aldın! Artık üzerine gönderilen tetikçilerin başarı şansı **%${PROTECTION_REDUCTION * 100}** oranında düşüyor.`);
        }
        return message.reply(`❌ Doğru kullanım: \`!koruma al\` (${PROTECTION_PRICE.toLocaleString()} FC, tetikçi başarı şansını yarıya indirir)`);
    }
    ...
    if (command === '!tetikçi' || command === '!tetikci') {
        ...
        const targetData = getUser(target.id);
        let basariSansi = (userData.mafia && mafias[userData.mafia]) ? MAFIA_TETIKCI_BASARI_SANSI : TETIKCI_BASARI_SANSI;
        if (targetData.protection) basariSansi *= PROTECTION_REDUCTION;
        const basarili = Math.random() < basariSansi;

        if (!basarili) {
            const korumaNotu = targetData.protection ? ' (koruması sayesinde atlattı!)' : '';
            return message.reply(`🚔 Tetikçi yakalandı, suikast başarısız oldu!${korumaNotu}`);
        }

...
function getUser(userId) {
    if (!db.users[userId]) {
        db.users[userId] = {
            ...
            marriedSince: null,
            ring: null,
            inventory: { altin: 0, gumus: 0, elmas: 0, uranyum: 0, dol: 0 },
            lastWork: 0
        };
    }
...
const MARKET = {
    ...
    uranyum: { name: 'Uranyum', emoji: '☢️', price: 9000 },
    dol: { name: 'Döl', emoji: '🧪', price: 300 }
};
...
client.on('messageCreate', async (message) => {
    ...
    if (command === '!yardım' || command === '!help') {
        const helpEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                {
                    name: '📈 Borsa',
                    value: '`!borsa` • Güncel fiyatları gösterir.\n`!yatırım al <altın/gümüş/elmas/uranyum/döl> <miktar>` • Yatırım yapar.\n`!yatırım sat <altın/gümüş/elmas/uranyum/döl> <miktar>` • Satış yapar.'
                },
                {
                    name: '💍 Evlilik',
    ...
    if (command === '!otoçalış' || command === '!otocalis') {
        ...
        const intervalId = setInterval(() => {
            ...
            const zam = Math.pow(1.03, u.level - 1);
            const kazanc = Math.round(job.basePay * zam);
            u.balance += kazanc;
            u.lastWork = Date.now();
        }, 30 * 1000);
    ...
    if (command === '!yatırım' || command === '!yatirim') {
        ...
        const itemMap = { 'altın': 'altin', 'altin': 'altin', 'gümüş': 'gumus', 'gumus': 'gumus', 'elmas': 'elmas', 'uranyum': 'uranyum', 'döl': 'dol', 'dol': 'dol' };
        const item = itemMap[itemKey];

        if (!['al', 'sat'].includes(action) || !item || !miktar || miktar <= 0) {
            return message.reply('❌ Doğru kullanım: `!yatırım al/sat <altın/gümüş/elmas/uranyum/döl> <miktar>`');
        }
